...
## 0. 작업 시작 전 최우선 점검 — `.gitignore`

이 워크스페이스에서 어떤 작업(실습 정리, 아카이빙, 문서 갱신 등)을 시작하기 전에 **가장 먼저** 루트 `.gitignore`와 각 프로젝트별 `.gitignore`(`ai-Library/.gitignore` 등)가 현재 상태에 맞게 최신인지 점검한다.

- 새로 생긴 산출물 폴더(예: 업로드 파일 저장 폴더, 런타임 생성 캐시/로그)가 아직 무시 목록에 없다면 즉시 추가한다.
- `db.properties` 등 민감 정보 파일이 여전히 `.gitignore`에 등록되어 있고 실제로 git에 추적되지 않는지(`git check-ignore -v`) 함께 확인한다.
- 점검·갱신 후에는 8번 항목(Git 및 저장소 관리)의 원칙에 따라 `chore:` 커밋 메시지로 기록할 수 있도록 변경 사항을 명확히 남긴다.
...
**코드 수록 및 설명 원칙**:
- 그날 새로 작성하거나 수정한 코드는 `...`, `나머지 필드 매핑`, `getter/setter 생략` 등으로 축약하지 않고 **전문(全文)을 그대로 수록**한다. (getter/setter를 포함한 VO 클래스, 각 메서드의 모든 분기까지 전체 포함)
- 코드를 나열하는 데 그치지 않고, 각 코드 블록마다 **동작 원리**(요청이 들어와서 응답이 나가기까지의 흐름, 각 API/어노테이션이 내부적으로 무엇을 하는지, 왜 그렇게 구현했는지)를 함께 서술한다.
