...
public class StudentDAO {
	...
	public boolean updateStudent(StudentDTO updateData) {
		String query = "UPDATE students SET password = ?, phone = ?, phone = ? WHERE student_id = ?";
		try {
			Connection conn = DBConnection.getConnection();
			PreparedStatement pstmt = conn.prepareStatement(query);
	...
	public List<String> getDeptList() {
		...
		try {
			...
			System.out.println("학과명 검색 DB 오류");
			e.printStackTrace();
		}
	...
	public boolean insertStudent(StudentDTO registerStudent) {
		String query = "INSERT INTO students VALUES (?,?,?, (SELECT dept_code FROM departments WHERE dept_name = ?),?)";
		try {
			Connection conn = DBConnection.getConnection();
			PreparedStatement pstmt = conn.prepareStatement(query);
