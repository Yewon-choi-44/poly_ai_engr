...
| 구분 | 내용 |
|------|------|
| 🔧 변경 | `GEMINI.md` → `AGENTS.md` 파일명 변경 (여러 AI 코딩 도구가 공통으로 인식하는 관례를 따름, `git mv`로 이력 보존, 내용은 0번/5번 섹션 추가 외 기존 규칙 그대로 유지) |
| 🔧 변경 | `AGENTS.md` — "0. 작업 시작 전 최우선 점검(.gitignore·저장소 범위)"와 "5. AI 어시스턴트 행동 원칙" 섹션 신설 |
| 🔧 변경 | `README.md` — 워크스페이스 실습 관리 규칙 링크를 `AGENTS.md`로 갱신, 누락되어 있던 `flexblog/` 바로가기 링크 추가 |

> 이번 변경은 문서/설정 정리 목적이며, `daily_practice/`, `exam1/`, `exam2/`, `exam0902/`, `todolist/`, `flexblog/`, `PracticeFile/`, `notes/` 등 실습 코드·구조 자체는 2026-09-16 항목 이후 변동이 없습니다(작업 시작 전 `git status`로 확인). 전체 디렉토리 트리는 바로 아래 2026-09-16 항목 참고.
