...
from Own_ML_Class_kuffers import CNN
...
for epoch in range(20):
    ...
    for X, Y in train_loader:
        ...
        avg_loss += loss.item()
    print(f"epoch : {epoch}, loss : {avg_loss/len(train_loader)}")


# 둘 중 하나는 파라미터만 저장
torch.save(model, 'kuffers_cnn.pth') # 모델 아키텍처 + 파라미터 저장
torch.save(model.state_dict(), 'kuffers_cnn.pt') # 파라미터만 저장
